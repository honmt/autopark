...
// Обработка действий (удаление записей)
        ...
        case 'delete':
            switch ($entity) {
                ...
                case 'motorcycle':
                    ...
                    break;
                case 'service':
                    $stmt = $db->prepare("DELETE FROM motorcycle_services WHERE id = ?");
                    break;
                case 'trip':
                    $stmt = $db->prepare("DELETE FROM trips WHERE id = ?");
                    break;
                default:
                    $_SESSION['error'] = 'Неизвестная сущность';
...
function getAdminStats($db) {
    ...
    // Количество просроченных ТО (status = 'overdue')
    $stmt = $db->query("SELECT COUNT(*) as count FROM motorcycle_services WHERE status = 'overdue'");
    $stats['overdue_services'] = $stmt->fetch(PDO::FETCH_ASSOC)['count'];
    
    // Количество предстоящих ТО (status = 'upcoming')
    $stmt = $db->query("SELECT COUNT(*) as count FROM motorcycle_services WHERE status = 'upcoming'");
    $stats['upcoming_services'] = $stmt->fetch(PDO::FETCH_ASSOC)['count'];
    
    // Общий пробег всех мотоциклов - используем odometer
    ...
    // Количество поездок за последние 30 дней (используем trip_date)
    $stmt = $db->query("SELECT COUNT(*) as count FROM trips WHERE trip_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)");
...
function getRecentActivity($db) {
    ...
    // Последние 5 ТО
    $stmt = $db->query("SELECT s.*, CONCAT(m.make, ' ', m.model) as motorcycle FROM motorcycle_services s LEFT JOIN motorcycles m ON s.motorcycle_id = m.id ORDER BY s.created_at DESC LIMIT 5");
    ...
    // Последние 5 поездок (используем trip_date)
    $stmt = $db->query("SELECT t.*, CONCAT(m.make, ' ', m.model) as motorcycle FROM trips t LEFT JOIN motorcycles m ON t.motorcycle_id = m.id ORDER BY t.trip_date DESC LIMIT 5");
    ...
    <div class="admin-content">
        ...
        <div class="admin-topbar fade-in">
            ...
            <div class="topbar-actions">
                <button class="btn-admin btn-secondary" onclick="location.reload()">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="action-buttons">
                                    <a href="users.php?action=view&id=<?php echo $user['id']; ?>" class="action-btn view" title="Просмотр">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="users.php?action=edit&id=<?php echo $user['id']; ?>" class="action-btn edit" title="Редактировать">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="9" style="text-align: center; padding: 30px;">
                                <i class="fas fa-info-circle"></i> Нет данных о техническом обслуживании
                            </td>
                        </tr>
                        <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td><?php echo !empty($service['next_service_mileage']) ? number_format($service['next_service_mileage'], 0, '', ' ') . ' км' : '-'; ?></td>
                            <td>
                            ...
                            <td>
                                <div class="action-buttons">
                                    <a href="services.php?action=view&id=<?php echo $service['id']; ?>" class="action-btn view" title="Просмотр">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="services.php?action=edit&id=<?php echo $service['id']; ?>" class="action-btn edit" title="Редактировать">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form method="POST" class="delete-form" onsubmit="return confirm('Вы уверены, что хотите удалить эту запись ТО?')">
                                        <input type="hidden" name="action" value="delete">
        ...
        <!-- Последние поездки -->
        <div class="admin-table fade-in" style="animation-delay: 0.4s;">
            <div class="table-header">
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th>Мотоцикл</th>
                            <th>Дата</th>
                            <th>Начальный пробег</th>
                            <th>Конечный пробег</th>
                            <th>Расстояние</th>
                            <th>Описание</th>
                            <th>Действия</th>
                        </tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="action-buttons">
                                    <a href="trips.php?action=view&id=<?php echo $trip['id']; ?>" class="action-btn view" title="Просмотр">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="trips.php?action=edit&id=<?php echo $trip['id']; ?>" class="action-btn edit" title="Редактировать">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form method="POST" class="delete-form" onsubmit="return confirm('Вы уверены, что хотите удалить эту поездку?')">
                                        <input type="hidden" name="action" value="delete">
                                        <input type="hidden" name="entity" value="trip">
        ...
        <!-- Футер -->
        <div class="admin-footer fade-in" style="animation-delay: 0.5s;">
